...
hfont = pygame.font.Font('freesansbold.ttf', HINTFONT_SIZE)
...
def game(gamediff: str):
    ...
    while True:
        ...
        if gs.healthPoints <= 0:        # End the game if the number of tries goes down to zero, loss condition
            ...
            screen.blit(word, word.get_rect(center=(SCREEN_WIDTH/2, SCREEN_HEIGHT/2)))
            ...
            for i in range(min(hint_max, len(gs.currWord.clueList))):
                thisHint = hfont.render(gs.currWord.clueList[i], True, (0, 128, 0))
                screen.blit(thisHint, (SCREEN_WIDTH/20, 45 + i*15))    
